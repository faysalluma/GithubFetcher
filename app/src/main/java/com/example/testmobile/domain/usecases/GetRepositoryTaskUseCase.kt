...
@Singleton
class GetRepositoryTaskUseCase @Inject constructor(private val sampleRepository: ISampleRepository) : TaskUseCase<Boolean, Unit>() {
    override suspend fun run(param: Unit): Either<Failure, Boolean> {
        return sampleRepository.getAllRepositories()
    }
}